...
int main() {
  ...
  juce::File f(
      "../test/TestAudioLib1Data/Library/AudioAccessCounts/default_profile");
  f.deleteFile();
  // try to load the audio library in the test folder and test top 100 is right
  AudioLibraryManager *lib =
      new AudioLibraryManager("../test/TestAudioLib1Data/", "default");
  juce::File a("./a");
  juce::File b("./b");
  juce::File c("./c");
  lib->countAccess(a);
  ...
  if (ent[0] != a.getFullPathName()) {
    std::cerr << "first sorted entry is not a after save: " << ent[0]
              << std::endl;
    return 1;
  }
  if (ent[1] != b.getFullPathName()) {
    std::cerr << "first sorted entry is not b after save: " << ent[1]
              << std::endl;
    return 1;
  }
  if (ent[2] != c.getFullPathName()) {
    std::cerr << "first sorted entry is not c after save: " << ent[2]
              << std::endl;
    return 1;
  }
  ...
  for (size_t i = 0; i < 2000; i++) {
    testFiles.push_back(juce::File(
        "../test/TestAudioLib1Data/Library/TestSamples/" + std::to_string(i)));
  }
