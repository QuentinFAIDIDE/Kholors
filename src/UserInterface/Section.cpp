...
void drawSection(juce::Graphics &g, juce::Rectangle<int> &bounds, juce::String title, juce::Colour &background)
{
    ...
    g.fillRoundedRectangle(bounds.getX(), bounds.getY() + bounds.getHeight() - 6, bounds.getWidth(), 6, 4);

    g.fillRoundedRectangle(bounds.getX() + bounds.getWidth() - 6, bounds.getY() + 18, 6, bounds.getHeight() - 18 - 6,
                           4);
    ...
    g.setColour(juce::Colour(220, 220, 220));
    g.drawText(title, bounds.getX(), bounds.getY(), bounds.getWidth(), 18, juce::Justification::centred, false);
}