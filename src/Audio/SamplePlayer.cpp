...
void SamplePlayer::setBuffer(BufferPtr targetBuffer, juce::dsp::FFT& fft) {
  ...
  audioBufferFrequencies.resize(numChannels * numFft *
  ...
  juce::dsp::WindowingFunction<float> window(
      FREQVIEW_SAMPLE_FFT_SIZE, juce::dsp::WindowingFunction<float>::hann);
  ...
  for (size_t i = 0; i < numChannels; i++) {
    ...
    for (size_t j = 0; j < numFft; j++) {
      ...
      window.multiplyWithWindowingTable(&inputOutputData[0],
      ...
      for (size_t k = 0; k < FREQVIEW_SAMPLE_FFT_SCOPE_SIZE; k++) {
        ...
        auto logIndexFft = juce::jlimit(
            0, FREQVIEW_SAMPLE_FFT_SIZE / 2,
            (int)(log10Index * (float)FREQVIEW_SAMPLE_FFT_SIZE * 0.5f));
        audioBufferFrequencies[(((i * numFft) + j) *
                                FREQVIEW_SAMPLE_FFT_SCOPE_SIZE) +
                               k] = inputOutputData[logIndexFft];
      }
    }
  }
}

int SamplePlayer::getNumFft() const { return numFft; }
...
juce::int64 SamplePlayer::getNextReadPosition() const { return position; }

void SamplePlayer::setNextReadPosition(juce::int64 p) { position = p; }
...
void SamplePlayer::setLength(juce::int64 length) {
  const juce::SpinLock::ScopedLockType lock(playerMutex);
  if (bufferStart + length <
      audioBufferRef->getAudioSampleBuffer()->getNumSamples()) {
...
void SamplePlayer::prepareToPlay(int samplesPerBlockExpected,
                                 double sampleRate) {
  // TODO
}
